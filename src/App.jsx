...
/* =====================================================================
   WORLD CUP 2026 — Lịch & phân tích (giờ Việt Nam)
   Nguồn dữ liệu: API-Football (api-sports.io) qua hàm trung gian /api/football
...
// Đổi mốc thời gian ISO -> giờ Việt Nam (UTC+7)
function toVN(iso) {
...
const C = {
  bg: "#0B1120", card: "#121A2B", line: "#1E293B", line2: "#243049",
  text: "#E7ECF3", sub: "#7E8AA0", dim: "#5A6478", accent: "#E63946",
  gold: "#FFD166", green: "#4ADE80",
};
...
export default function App() {
  ...
  // Tải toàn bộ lịch giải, gom theo bảng
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      // Gom theo vòng/bảng. API trả round dạng "Group A - 1"
      const map = {};
      for (const f of fixtures) {
        const round = f.league?.round || "";
        const gm = round.match(/Group ([A-L])/i);
        const key = gm ? gm[1].toUpperCase() : "KO"; // KO = vòng loại trực tiếp
        (map[key] ||= []).push(f);
      }
      setGroups(map);
    } catch (e) {
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "'Inter',system-ui,sans-serif" }}>
      <style>{`
        *{box-sizing:border-box}
        ...
        .card:hover{transform:translateY(-2px);border-color:${C.accent}!important}
        @keyframes spin{to{transform:rotate(360deg)}}
        .spin{animation:spin 1s linear infinite;display:inline-block}
        .pill{font-size:11px;font-weight:700;letter-spacing:.4px;padding:3px 9px;border-radius:999px}
      ...
      <header style={{ borderBottom: `1px solid ${C.line}`, padding: "16px", display: "flex", alignItems: "center", gap: 12, position: "sticky", top: 0, background: "rgba(11,17,32,.92)", backdropFilter: "blur(8px)", zIndex: 10 }}>
        {view.name !== "groups" && (
          <button onClick={() => setView(view.name === "match" ? { name: "group", g: view.g } : { name: "groups" })}
            style={{ background: "none", border: "none", color: C.accent, cursor: "pointer", fontSize: 24, lineHeight: 1 }}>‹</button>
        )}
...
/* -------- Màn 1: danh sách bảng -------- */
function Groups({ groups, onOpen }) {
  const ids = Object.keys(groups).filter((k) => k !== "KO").sort();
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(220px,1fr))", gap: 12 }}>
      {ids.map((id) => {
        ...
        return (
          <button key={id} onClick={() => onOpen(id)} className="card"
            style={{ textAlign: "left", cursor: "pointer", background: C.card, border: `1px solid ${C.line}`, borderRadius: 14, padding: 14, color: "inherit" }}>
            ...
            {teams.map((t) => (
              <div key={t.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "3px 0", fontSize: 14 }}>
                <img src={t.logo} alt="" width={18} height={18} style={{ objectFit: "contain" }} />
                <span>{t.name}</span>
              </div>
            ))}
          </button>
    ...
    <div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {sorted.map((m) => {
          ...
          return (
            <button key={m.fixture.id} onClick={() => onOpenMatch(m)} className="card"
              style={{ textAlign: "left", cursor: "pointer", background: C.card, border: `1px solid ${C.line}`, borderRadius: 14, padding: 14, color: "inherit" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                <span style={{ fontSize: 12, color: C.sub }}>📅 {t.date} &nbsp;🕒 {t.time}</span>
                <span className="pill" style={done ? { background: "rgba(34,197,94,.15)", color: C.green } : { background: "rgba(230,57,70,.15)", color: "#FF6B7A" }}>
                  {done ? "ĐÃ ĐÁ" : "CHƯA ĐÁ"}
                </span>
              </div>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                <span style={{ flex: 1, textAlign: "right", fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 6 }}>
                  {m.teams.home.name} <img src={m.teams.home.logo} width={22} height={22} alt="" />
                </span>
                <span style={{ minWidth: 56, textAlign: "center", fontWeight: 800, color: done ? C.gold : C.dim }}>{done ? sc : "vs"}</span>
                <span style={{ flex: 1, textAlign: "left", fontWeight: 700, display: "flex", alignItems: "center", gap: 6 }}>
                  <img src={m.teams.away.logo} width={22} height={22} alt="" /> {m.teams.away.name}
                </span>
              </div>
  ...
  const load = useCallback(async () => {
    ...
    try {
      if (done) {
        ...
        const j = await r.json();
        setPred(j.response?.[0] || null);
      }
    } catch (e) {
      setErr("Không tải được chi tiết. " + e.message);
    ...
    <div>
      <div style={{ background: "linear-gradient(135deg,#15203A,#101727)", border: `1px solid ${C.line2}`, borderRadius: 16, padding: "20px 16px", marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
          ...
          <div style={{ fontSize: 30, fontWeight: 900, color: done ? C.gold : C.dim, minWidth: 70, textAlign: "center" }}>
      ...
      {done && !loading && stats && (
        <div style={{ background: C.card, border: `1px solid ${C.line}`, borderRadius: 14, padding: 14 }}>
          ...
          {[
            ["Sút trúng đích", "Shots on Goal"],
            ["Phạt góc", "Corner Kicks"],
            ["Thẻ vàng", "Yellow Cards"],
            ["Thẻ đỏ", "Red Cards"],
            ["Kiểm soát bóng", "Ball Possession"],
          ].map(([label, type]) => (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {pred.teams && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <FormBox t={pred.teams.home} />
              <FormBox t={pred.teams.away} />
            </div>
          )}
        </div>
      )}
      {!done && !loading && !pred && !err && (
        <div style={{ color: C.sub, fontSize: 13, textAlign: "center", padding: 20 }}>Chưa có dữ liệu dự đoán cho trận này.</div>
      )}
    </div>
  );
}
...
function Side({ team }) {
  return (
    <div style={{ flex: 1, textAlign: "center" }}>
      <img src={team.logo} width={44} height={44} alt="" style={{ objectFit: "contain" }} />
      <div style={{ fontWeight: 700, marginTop: 4, fontSize: 14 }}>{team.name}</div>
    </div>
  );
}
function Pred({ label, value }) {
  return <div><div style={{ fontSize: 11, color: C.sub, marginBottom: 4 }}>{label}</div><div style={{ fontWeight: 800, fontSize: 16, color: C.gold }}>{value || "—"}</div></div>;
}
function FormBox({ t }) {
  return (
    <div style={{ background: C.card, border: `1px solid ${C.line}`, borderRadius: 14, padding: 14 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, fontWeight: 800, marginBottom: 8 }}>
        <img src={t.logo} width={20} height={20} alt="" /> {t.name}
      </div>
